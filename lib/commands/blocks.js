...
(╯°□°）╯︵ ┻━┻
ZURB every time they change the URL...

 ¯\_(ツ)_/¯
People who don't realize every DNS change to the host URL,
will probably break the already dated, yet powerful CLI...

┬─┬﻿ ノ( ゜-゜ノ)
Little old Me trying to maintain my own already buggy frankenstein of a fork...

(｡◕‿◕｡)
My poor 16-18 year old coding students confused and blaming themselves...

¯\_(⊙︿⊙)_/¯
Me trying to explain to them...

 */
const hostURL = "get.foundation";
...
function listBuildingBlocks(args, options, callback) {
  fetchUrl(`${hostURL}`, '/building-blocks/data/building-blocks.json', function(blocksJson) {
    var i = 0;
    var index = "";
...
function installFiles(name, callback) {
  async.map(FILETYPES, function(filetype, cb) {
    ...
    fs.mkdir(filetype.directory, function() {
      fetchUrl(`${hostURL}`, path, function(file) {
        if(file.length > 0) {
