...
var repositories = {
  sites: {
    ...
    styleguide: 'https://github.com/HansUXdev/Resposive-StyleGuide',
    zurb: 'https://github.com/zurb/foundation-zurb-template.git',
  },
  apps: {
    angular: 'https://github.com/zurb/foundation-apps-template.git',
    react: 'https://github.com/digiaonline/react-foundation',
    express: 'https://github.com/HansUXdev/zurb-express',
  },
  emails: {
    basic: 'https://github.com/zurb/foundation-emails-template.git',
    // server: 'https://github.com/zurb/foundation-emails-template.git',
  },
  wordpress: {
    FoundationPress: 'https://github.com/zurb/foundation-emails-template.git',
    JointsWP: 'https://github.com/JeremyEnglert/JointsWP',
  }
}

module.exports = function(args, options, callback, ee) {
  var projectName, projectFolder, framework, sites_templates, app_templates, email_templates,WP_templates, messages, directory;
  var tasks = [
  ...
  function prompt(cb) {
    inquirer.prompt(util.questions(options), function(answers) {
      ...
      sites_templates = answers.sites_templates || options.sites_templates || 'unspecified';
      app_templates = answers.app_templates || options.app_templates || 'unspecified';
      email_templates = answers.email_templates || options.email_templates || 'unspecified';
      WP_templates = answers.WP_templates || options.WP_templates || 'unspecified';

      projectFolder = path.join(process.cwd(), projectName);

      messages = util.messages(projectName,framework,sites_templates,app_templates,email_templates,WP_templates);
  ...
  function gitClone(cb) {
    // This should not use a goddamn ternary...
    var repo = framework === 'sites'
      ? repositories.sites[sites_templates]
      : repositories.apps[app_templates] || repositories.emails[email_templates] || repositories.wordpress[WP_templates] || repositories[framework];
      /*
      // switch(answers.framework) {
      //     case "sites": repositories.sites[sites_templates];
      //         break;
      //     case "apps": repositories.apps[app_templates];
      //         break;
      //     case "emails": repositories.emails[email_templates];
      //         break;
      //     default:
      //         repositories.sites[sites_templates];
      // };
